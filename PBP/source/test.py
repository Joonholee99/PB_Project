...
class TestStrategy(bt.Strategy):
    def __init__(self):
        bt.indicators.SimpleMovingAverage(self.data, period=5)  # period=5 찾아낸 법
    ...
    def notify_order(self, order):
        if order.status in [
            order.Submitted,
            order.Accepted,
        ]:  # Submitted : 브로커한테 주문 보냄 / Accepted : 브로커가 주문 받음 / 이 두 시간 차이로 나는 손해가 Slippage
            return
        elif order.status in [order.Completed]:
            if order.isbuy():
                self.meanPrice = (
                    self.meanPrice * self.numberOfStocks
                    + order.size * self.dataclose[0]
    ...
    def next(self):
        ...
        if self.i < 20:
            ...
            if (
                MA5 > MA20
                and self.pastMA5 < self.pastMA20
                and self.broker.getcash() > self.dataclose[0]
            ):
                amountToOrder = int(self.broker.getcash() / self.dataclose[0])
                self.order = self.buy(size=amountToOrder)
                print(
                    self.i,
                    " - BUY : {}, buy amount : {}".format(
                        self.dataclose[0], amountToOrder
                    ),
                )

            elif (
                self.meanPrice * 1.1 < self.dataclose[0]
                and self.numberOfStocks is not 0
            ):
                print(
                    self.i,
                    " - SELL : {}, sell amount : {}".format(
                        self.dataclose[0], self.numberOfStocks
                    ),
                )
...
stockData = web.DataReader(
    "AAPL", "yahoo", datetime.datetime(2018, 2, 20), datetime.datetime(2021, 2, 20)
)[["Open", "Close"]]
